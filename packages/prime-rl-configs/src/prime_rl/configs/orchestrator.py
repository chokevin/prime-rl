...
class PrimeAwareRequestPickerConfig(BaseConfig):
    ...
    """External Prime scheduler URL. If unset, use the in-process picker."""

    group_tail_weight: float = Field(0.0, ge=0)
    oldest_group_tail_weight: float = Field(0.0, ge=0)
    queue_penalty_weight: float = Field(0.0, ge=0)
    completed_rps_deficit_weight: float = Field(0.0, ge=0)
    waiting_backpressure_threshold: int = Field(0, ge=0)
    waiting_backpressure_penalty: float = Field(0.0, ge=0)
    decode_guardrail_ratio: float = Field(0.0, ge=0)
    decode_guardrail_penalty: float = Field(0.0, ge=0)
    long_output_cold_start_ratio: float = Field(0.0, ge=0)
    long_output_cold_start_min_samples: int = Field(0, ge=0)
    wave_minimax_size: int = Field(0, ge=0)
    wave_overhang_limit: int = Field(0, ge=0)
    wave_overhang_start_progress: float = Field(0.0, ge=0, le=1)


RequestPickerConfig: TypeAlias = Annotated[
    DirectRequestPickerConfig | PrimeAwareRequestPickerConfig, Field(discriminator="type")
]


class OrchestratorExperimentalConfig(BaseConfig):
    request_picker: RequestPickerConfig = DirectRequestPickerConfig()
    """Experimental request picker used to assign rollout requests to inference clients."""
...
class OrchestratorConfig(BaseConfig):
    ...
    """Dispatch rounds with failed/empty rollouts before dropping a stuck group. None retries indefinitely."""

    max_async_level: int = Field(1, ge=1)
    """Maximum number of trainer steps the orchestrator may run ahead."""

    strict_async_level: bool = False
    """When true, require exactly max_async_level lag; otherwise use the newest compatible checkpoint."""
